...
If you include ``*_roi_paths``, **only the atlases and analyses provided** will be used, unless you add the optional key/value pair ``roi_paths_fully_specified: False`` in the relevant section, in which case the default atlases and analyses will be updated with the atlases and analyses specifed in your pipeline config. For example,
